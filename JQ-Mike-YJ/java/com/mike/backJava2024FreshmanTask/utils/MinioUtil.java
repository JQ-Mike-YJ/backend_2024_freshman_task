...
public class MinioUtil {
    ...
    public void existBucket(String name) {
        try {
            ...
            log.error("minioUtil existBucket error -> {}", e.getMessage(), e);
        }
    }
    ...
    public String upload(MultipartFile file) {
        ...
        try {
            ...
            log.error("minioUtil upload error -> {}", e.getMessage(), e);
        } finally {
            if (in != null) {
                try {
                    in.close();
     ...
     * @return String 文件minio地址
     * @throws Exception 异常
     */
    public String exportToMinio(List<List<Object>> dataList, String fileNamePrefix) throws Exception {

        // 生成 Excel 文件
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        EasyExcel.write(out).sheet("会议报名信息").doWrite(dataList);

        // 文件名称为空，设置默认的名称
        String fileName = fileNamePrefix + System.currentTimeMillis() + ".xlsx";

        // 转换为 MultipartFile
        FileItem fileItem = new DiskFileItem("file", "application/vnd.ms-excel", true, fileName, 1024, null);
        fileItem.getOutputStream().write(out.toByteArray());
    ...
    public ObjectWriteResponse uploadFile(String bucketName, String objectName, InputStream inputStream) {
        return minioClient.putObject(PutObjectArgs.builder().bucket(bucketName).object(objectName).stream(inputStream,
     ...
     */
    @SneakyThrows(Exception.class)
    public String getPresignedObjectUrl(String bucketName, String objectName) throws ServerException, InsufficientDataException,
            ErrorResponseException, IOException, NoSuchAlgorithmException, InvalidKeyException, InvalidResponseException, XmlParserException,
            InternalException {
